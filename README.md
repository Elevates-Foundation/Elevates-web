...
## Connection to Elevates OS

This site is the **public marketing surface**. Admin / HQ / chapter consoles live in [Elevates OS](https://github.com/Elevates-Foundation/Elevates-os) (`os.elevates.live`).

| Do | Don't |
| --- | --- |
| Fetch public data via `OS_API_URL` (`src/lib/os-client.ts`) | Add `/hq`, `/chapter`, or Supabase service role here |
| Fall back to `src/data/*` when OS is down | Evaluate RBAC permissions on the web |
| Host `/verify/certificate/[id]` on this domain | Put write business logic outside OS |

```bash
cp .env.example .env.local
# OS_API_URL=https://os.elevates.live/api/public/v1
# NEXT_PUBLIC_USE_LIVE_DATA=true   # after OS public API is deployed
