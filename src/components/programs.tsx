...
export default function Programs() {
    return (
        <section className="h-screen bg-graphite text-paper relative flex flex-col justify-center">
            ...
            {/* Horizontal Slider */}
            <div className="flex gap-8 px-4 md:px-10 items-center w-full h-full pt-32 pb-10 overflow-x-auto snap-x snap-mandatory relative z-10 scroll-smooth" style={{ scrollbarWidth: 'thin', scrollbarColor: '#f26430 transparent' }}>
                    ...
                    >
                        ...
                        <div className="mt-4 pt-4 border-t-2 border-dashed border-graphite/30 flex justify-between items-end relative z-10">
                            <div className="flex flex-col">
                                <span className="font-mono text-[10px] text-olive uppercase tracking-tight">CURRENT STATUS</span>
                                <span className={`font-black text-lg md:text-xl uppercase ${prog.status === 'COMPLETED' ? 'text-olive line-through decoration-2' : 'text-flame'}`}>
                                    {prog.status}
                                </span>
                            </div>
                            ...
                            <div className="w-12 h-12 border-2 border-graphite p-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                <div className="w-full h-full bg-graphite/20 grid grid-cols-4 grid-rows-4 gap-0.5">
                                    {[...Array(16)].map((_, k) => (
                                        <div key={k} className={`bg-graphite ${(k * 7 + i * 3) % 5 > 2 ? 'opacity-100' : 'opacity-0'}`} />
                                    ))}
                                </div>
                            </div>
                        </div>
                        ...
                        <Doodle type="scribble" color="#f26430" className="absolute bottom-20 right-4 w-32 h-32 opacity-0 group-hover:opacity-10 rotate-12 transition-opacity pointer-events-none" />
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 text-olive opacity-80 z-20 pointer-events-none">
                ...
                <span className="font-mono text-xs uppercase tracking-widest">Scroll / Swipe Through</span>
